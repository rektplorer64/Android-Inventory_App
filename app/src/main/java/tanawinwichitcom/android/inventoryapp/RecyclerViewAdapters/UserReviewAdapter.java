...
public class UserReviewAdapter extends RecyclerView.Adapter<UserReviewAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull UserReviewAdapter.ViewHolder holder, int position){
        ...
        DateFormat dateFormat = new SimpleDateFormat("dd/MM/YYYY", HelperUtilities.getCurrentLocale(holder.cardView.getContext()));
        if(user != null){
            ...
            long timeStamp = Long.valueOf(review.getTimeStamp());
            holder.userReviewDateTextView.setText(dateFormat.format(new Date(timeStamp)));
